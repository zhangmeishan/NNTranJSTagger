...
int JSTagger::createAlphabet(const vector<Instance>& vecInsts) {
	...
	m_driver._hyperparams.postags.clear();
    ...
    for (numInstance = 0; numInstance < vecInsts.size(); numInstance++) {
		...
		for (int idx = 0; idx < instance.wordsize(); idx++) {
			m_driver._hyperparams.postags.from_string(instance.tags[idx]);
			postag_stat[instance.tags[idx]]++;
		}
        count += instance.wordsize();
    }

	m_driver._hyperparams.postags.set_fixed_flag(true);
    ...
    if (!m_options.wordEmbFineTune && m_options.wordEmbFile != "") {
        m_driver._modelparams.embeded_ngram_chars.initial(m_options.wordEmbFile);
    }
	else {
		cout << "no external word embedding file" << std::endl;
	}
...
void JSTagger::train(const string& trainFile, const string& devFile, const string& testFile, const string& modelFile, const string& optionFile) {
    ...
    if (m_options.wordEmbFile != "") {
        initial_successed = m_driver._modelparams.ngram_char_table.initial(&m_driver._modelparams.embeded_ngram_chars, m_options.wordEmbFile, false, m_options.wordEmbNormalize);
        if (initial_successed) {
            m_options.wordEmbSize = m_driver._modelparams.ngram_char_table.nDim;
        }
    }
	else {
		cout << "no external word embedding file" << std::endl;
		m_options.wordEmbSize = 0;
	}
