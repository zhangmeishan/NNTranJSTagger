...
class Pipe {
  public:
    ...
    int outputAllInstances(const string& m_strOutFile, const vector<vector<string> >& seg_results, const vector<vector<string> >& tag_results) {
        initOutputFile(m_strOutFile.c_str());
        static int instNum;
        instNum = seg_results.size();
        for (int idx = 0; idx < instNum; idx++) {
            if (0 != m_jstWriter->write(seg_results[idx], tag_results[idx]))
                return -1;
        }
