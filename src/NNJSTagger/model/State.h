...
class CStateItem {
  ...
  public:
    ...
    void getResults(std::vector<std::string>& words, std::vector<std::string>& tags, HyperParams* opt) const {
        ...
        for (int idx = 0; idx < state_num; idx++) {
            ...
            tags.push_back(opt->postags.from_id(preSepStates[idx]->_tag));
        }
    }
    ...
    void getGoldAction(const Instance& inst, CAction& ac, HyperParams* opt) const {
        ...
        if (_next_index == 0) {
            ac.set(CAction::SEP, opt->postags.from_string(inst.tags[_word_count]));
            return;
        }
  ...
  public:
    ...
    inline void prepare(HyperParams* hyper_params, ModelParams* model_params, GlobalNodes* global_nodes) {
        ...
        _atomFeat.str_tag = _tag >= 0 ? hyper_params->postags.from_id(_tag) : nullkey;
        ...
        _atomFeat.p_bichar_left_lstm = global_nodes == NULL ? NULL : &(global_nodes->bichar_left_lstm);
        _atomFeat.p_bichar_right_lstm = global_nodes == NULL ? NULL : &(global_nodes->bichar_right_lstm);
    }
};
